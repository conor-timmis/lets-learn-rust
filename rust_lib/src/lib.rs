...
mod tests {
    ...
    pub fn calculate_stats(numbers_str: &str) -> String {
        ...
        if numbers.is_empty() {
            // This case might occur if input was just commas or whitespace producing no valid numbers after split and parse
        ...
        // .min() and .max() return Option<&i32>, so we unwrap (safe because we checked numbers.is_empty())
